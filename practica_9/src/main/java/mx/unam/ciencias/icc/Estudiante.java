...
public class Estudiante implements Registro<Estudiante, CampoEstudiante> {
    ...
    @Override public boolean equals(Object objeto) {
        ...
        return estudiante != null &&
            ...
            cuenta.get() == estudiante.cuenta.get() &&
    ...
    @Override public String aLinea() {
        return String.format("%s\t%d\t%2.2f\t%d\n", nombre.get(), cuenta.get(),
                                promedio.get(), edad.get());
    }
    ...
    @Override public void deLinea(String linea) {
        ...
        if (partes.length != 4)
            throw new ExcepcionLineaInvalida("La línea no contiene los 4 elementos requeridos por el Estudiante.");
        ...
        try {
            ...
            throw new ExcepcionLineaInvalida("No es posible hacer la conversión a int o double.");
        }

        nombre.set(partes[0]);
        promedio.set(promedio_nuevo);
        cuenta.set(cuenta_nuevo);
        edad.set(edad_nuevo);
    }
    ...
    public void actualiza(Estudiante estudiante) {
        if (estudiante == null)
            throw new IllegalArgumentException("El estudiante es inválido.");
    ...
    private boolean cazaCuenta(Object valor) {
        ...
        int cuentaValor = (int)valor;
        return cuentaValor <= cuenta.get();
    }
    ...
    private boolean cazaPromedio(Object valor) {
        ...
        double promedioValor = (double)valor;
        return promedioValor <= promedio.get();
    }
    ...
    private boolean cazaEdad(Object valor) {
        ...
        int edadValor = (int)valor;
        return edadValor <= edad.get();
    }
}
